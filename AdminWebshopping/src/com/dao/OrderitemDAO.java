...
public class OrderitemDAO {
	 ...
	 */
	public static boolean newOrderItems(List<Orderitem> list, int OrderID) {
		boolean back = false;

		Connection con = DAO.getCon();
		try {
			Iterator<Orderitem> li = list.iterator();
	...
	public static List<Orderitem> getOrders(int OrderID) throws SQLException {
		...
		Connection con = DAO.getCon();
		String sqlstr = "select order_id,goods_id,goods_quantity,goodsDtail,goods.name,goods.image,price from orderitem join goods on orderitem.goods_id=goods.goodID where "
				+ " order_id = ?";
