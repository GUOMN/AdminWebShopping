...
public class PageBar {
	...
	/** ③-生成分页栏 */
	public void setPageBar(String strcurrentP, String strcurrentG,
			String goWhich) {
		setAllP(); // 设置总记录数
		setAllG(); // 设置总组数
	...
	private void setPageLink(String goWhich) {
		...
		if (currentG > 1)
			pageLink += "<a href='" + goWhich + "currentG=" + (currentG - 1)
					+ "' class='pagertext'>上" + perP + "页</a> ";
		if (currentP > 1) {
			pageLink += "<a href='" + goWhich + "currentP=1'>首页</a> ";
			pageLink += "<a href='" + goWhich + "currentP=" + (currentP - 1)
					+ "'>上一页</a> | ";
		}

		if (currentP % perP == 0)
			pageLink += "<a class='pagerCurrentP'>" + currentP
					+ "</a>&nbsp;&nbsp;";
		...
		for (int i = 0; ((i < perP) && ((start + i) <= allP)); i++) {
			if ((start + i) % perP == 0)
				...
				temp = currentG;
			if ((start + i) == currentP)
				pageLink += "<a class='pagerCurrentP'>" + (start + i)
						+ "</a>&nbsp;&nbsp;";
			else
				pageLink += "<a href='" + goWhich + "currentP=" + (start + i)
						+ "' class='pagerline'>" + (start + i)
						+ "</a>&nbsp;&nbsp;";
		}
		...
		if (currentP < allP) {
			pageLink += "| <a href='" + goWhich + "currentP=" + (currentP + 1)
					+ "'>下一页</a>";
			pageLink += " <a href='" + goWhich + "currentP=" + allP
					+ "#listtop'>尾页</a>";
		}
		if (currentG < allG)
			pageLink += " <a href='" + goWhich + "currentG=" + (currentG + 1)
					+ "' class='pagertext'>下" + perP + "页</a>";
